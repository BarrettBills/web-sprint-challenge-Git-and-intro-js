...
function listOfNames(array) {
  ...
  for (let i = 0; i < newArray1.length; i++) {
    if(newArray1[i] !== "name") {
      newestArray1.push(newArray1[i].name);
      }
    }
    return newestArray1;
  }
...
console.log(addArtist(artists, "Barrett Bills", "1991 - 2022", "Web Design", "Mexican American", "Barrett was born and raised in the midwest of the United States. Having moved west and starting a family with his newfound wife, he began his dive into a career in web design."));
...
function lotsOfArt(array) {
  const overachievingArtists = [];
  for (let i = 0; i < array.length; i++) {
    if (array[i]['paintings'] > 100) {
      overachievingArtists.push(array[i]['name']);
    }
  }  
  return overachievingArtists;
}
